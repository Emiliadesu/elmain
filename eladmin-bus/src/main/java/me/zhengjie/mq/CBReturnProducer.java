...
@Slf4j
public class CBReturnProducer implements Producer{
    ...
    public void start() {
        if (!StringUtil.equals("dev", env)) {
            if (null == this.properties) {
                throw new ONSClientException("退货消息队列生产者报错：properties not set");
            } else {
                this.producer = ONSFactory.createProducer(this.properties);
                this.producer.start();
                log.info("退货消息队列生产者启动成功，环境：" + this.env + "TOPIC:" + this.topic );
            }
        }else {
            log.info("非正式环境不启动MQ...");
        }
